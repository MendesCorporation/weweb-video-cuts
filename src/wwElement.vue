<template>
  ...
  >
    ...
    <div v-if="videoDuration > 0" class="timeline-container" :style="timelineContainerStyle">
      ...
      <div ref="timelineRef" class="timeline-track" @pointerdown="handleTimelineClick">
        <div class="timeline-selection" :style="selectionStyle" @pointerdown.stop="handleSelectionDrag" />

        <div class="timeline-handle start" :style="startHandleStyle" @pointerdown.stop="handleStartDrag">
          <div class="handle-grip" />
        </div>

        <div class="timeline-handle end" :style="endHandleStyle" @pointerdown.stop="handleEndDrag">
          <div class="handle-grip" />
        </div>
      </div>
...
export default {
  ...
  setup(props, { emit }) {
    ...
    const { setValue: setCutStartTime } = wwLib.wwVariable.useComponentVariable({
      uid: props.uid,
      name: 'cutStartTime',
      type: 'number',
      defaultValue: 0,
    ...
    const { setValue: setCutEndTime } = wwLib.wwVariable.useComponentVariable({
      uid: props.uid,
      name: 'cutEndTime',
      type: 'number',
      defaultValue: 0,
    ...
    const { setValue: setCutDuration } = wwLib.wwVariable.useComponentVariable({
      uid: props.uid,
      name: 'cutDuration',
      type: 'number',
      defaultValue: 0,
    ...
    const { setValue: setVideoReady } = wwLib.wwVariable.useComponentVariable({
      uid: props.uid,
      name: 'videoReady',
      type: 'boolean',
      defaultValue: false,
    ...
    const effectiveMaxDuration = computed(() => {
      ...
      if (videoDuration.value > 0) return Math.min(max, videoDuration.value);
      return max;
    });

    const effectiveMinDuration = computed(() => {
      const min = props.content?.minCutDuration ?? 0.5;
      if (videoDuration.value > 0) return Math.min(min, videoDuration.value);
      return min;
    });
    ...
    const handleSize = computed(() => {
      ...
      if (duration > 300) return { width: '12px', height: '32px' };
      if (duration > 120) return { width: '16px', height: '36px' };
    ...
    const gridWatermarkPositions = computed(() => {
      const spacing = Math.max(50, gridSpacing.value || 150);
      const positions = [];
    ...
    const handleVideoError = () => {
      ...
      setVideoReady(false);
      emit('trigger-event', { name: 'video-error', event: { error: 'Failed to load video' } });
    };

    const handlePlay = () => {
      isPlaying.value = true;
      emit('trigger-event', { name: 'play', event: { currentTime: videoRef.value?.currentTime || 0 } });
    };

    const handlePause = () => {
      isPlaying.value = false;
      emit('trigger-event', { name: 'pause', event: { currentTime: videoRef.value?.currentTime || 0 } });
    };
    ...
    const applyTimeMask = (value) => {
      ...
      if (numbers.length === 3) return `${numbers[0]}:${numbers.slice(1, 3)}`;
...
.video-wrapper {
  ...
  /* Isso ajuda muito no iOS: cria stacking context previsível */
  isolation: isolate;
...
/* No iOS, quando usamos controles custom, o vídeo não pode roubar tap/camada */
.video-cut-player.use-custom-controls .video-element {
  pointer-events: none;
}

/* Custom Controls sempre acima */
.custom-controls {
  position: absolute;
  left: 0;
  right: 0;
...
/* Watermark acima do vídeo, mas abaixo dos controles */
.watermark-grid {
  position: absolute;
  top: 0;
  left: 0;
